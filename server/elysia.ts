...
const gateback = new Elysia({ prefix: "/_gateback" })
  ...
  .onAfterResponse({ as: "global" }, async ({ request, set }) => {
    try {
      ...
      // /metrics 경로는 메트릭 수집 및 세션 체크에서 제외
      ...
      // 인증 이벤트 메트릭은 middleware/auth.ts에서 처리됩니다
...
const app = new Elysia()
  // /metrics는 server/index.ts의 Bun.serve routes에서 직접 처리됩니다
  .use(gateback)
  // gateback 이외 경로도 포함해 전체 요청에 대한 지연시간만 기록합니다.
  // (요청수/상태코드 카운트는 gateback/proxy에서 더 정확하게 기록)
  .onRequest(({ request }) => {
    const url = new URL(request.url);
    // /metrics 경로는 메트릭 수집에서 제외 (프로메테우스 스크랩 노이즈 방지)
    if (url.pathname === "/metrics") return;
